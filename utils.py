...
class Dataloader():
    ...
    def __init__(self):
        ...
        self.cluster_col_names = ['timestamp', 'x', 'y', 'velocity']
        self.plot_cluster_col_names = ['Row_ID', 'frame_num', 'ped_id', 'x', 'y', 'cluster', 'Traj_num', 'velocity']
        self.data_precessing_path = "pre_processed"
        self.retrieved_dataset = ''
        self.seq_lenght = 20


    def build_dataframes(self, force_preprocess = True, percent = 100):
        if force_preprocess:
            for dataset in self.datasets:
                self.process_dataset(dataset, percent)
        else:
            for dataset in self.datasets:
                try:
        ...
        for index, dataframe in enumerate(self.df_list):
            minpts, spatial, temporal, velocity = self.get_dbscan_params(dataframe)
            self.run_dbscan_on_dataframe(dataframe, minpts, spatial, temporal, velocity)
            self.dbscan_parameter[self.datasets[index]] = [minpts, spatial, temporal, velocity]
    ...
    def process_dataset(self, dataset, percent):
        self.df_list = [];
        ...
        df = (df.groupby('Traj_num').apply(lambda x : self.calculate_dist(x)))
        print(df.tail(60))
        df = self.take_percentage_df(df, percent)
        self.df_list.append(df)
    ...
    def run_dbscan_on_dataframe(self, dataframe, minPts, spatial_eps, temporal_eps, velocity_eps):
        if len(dataframe) is not 0:
            #print(self.cluster_col_names)
            dataframe = st_dbscan(dataframe, self.cluster_col_names, spatial_eps, temporal_eps, velocity_eps,minPts)

    def calculate_dist(self, x):
        p1 = np.array((x.iloc[0]['x'], x.iloc[0]['y']))
        p2 = np.array((x.iloc[-1]['x'], x.iloc[-1]['y']))
        velocity = np.linalg.norm(p1-p2)/self.seq_lenght
        x['velocity'] = velocity
        return x
    ...
    def get_percentage_data(self, percent):
        ...
        minpts, spatial, temporal, velocity_eps = self.get_dbscan_params(df_c)
        self.run_dbscan_on_dataframe(df_c , minpts, spatial, temporal, velocity_eps)
        self.dbscan_parameter[self.retrieved_dataset] = [minpts, spatial, temporal, velocity_eps]
        json_object = df_c[self.plot_cluster_col_names].to_json(orient = 'values')
        return json_object

    def run_dbscan_with_params(self ,pts, spatial, temporal, velocity_eps, percent):
        df = self.get_dataframe(self.retrieved_dataset)
        df = self.take_percentage_df(df, percent)
        self.run_dbscan_on_dataframe(df , pts, spatial, temporal, velocity_eps);
        self.dbscan_parameter[self.retrieved_dataset] = [pts, spatial, temporal, velocity_eps]
        json_object = df[self.plot_cluster_col_names].to_json(orient = 'values')
        return json_object
    ...
    def get_dbscan_params(self, dataframe):
        ...
        temporal = 4000
        velocity = 0.042
        return minpts, spatial, temporal, velocity
